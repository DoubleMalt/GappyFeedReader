	...
	var _process = function(e, data, options) {
		...
		for (var i=options.offset; i<feeds.entries.length; i++) {
			...
			if (entry.publishedDate) {
				...
				{
					var pubDate = $.format.date(entryDate.getTime(), options.dateFormat);
				}
				else
				{
			...
			if(options.title)
			{
				rowArray[rowIndex]['html'] += '<'+ options.titletag +'>' + 
						'<a href="'+ options.linkredirect + feedLink +
						'" title="View this feed at '+ feeds.title +'">'+ 
						entry.title +'</a></'+ options.titletag +'>';
			}

			if (options.date && pubDate) rowArray[rowIndex]['html'] += '<div class="rssItemDate">'+ pubDate +'</div>'
			if (options.content) {
